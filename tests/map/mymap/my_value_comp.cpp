...
int main()
{
	...
	ft::map<std::string, int>::value_compare a_valuecomp = a_map.value_comp();
	a_map["Paris"] = 42;
	a_map["Quebec"] = 42;
	a_map["Kazan"] = 21;
	a_map["Moscow"] = 21;
	...
	for (ft::map<std::string, int>::iterator it = a_map.begin(); a_valuecomp(*it, *a_map.rbegin()); *it++)
	...
	ft::map<int, std::string>::value_compare z_valuecomp = z_map.value_comp();
	z_map[88] = "Mercure";
	z_map[225] = "Venus";
	z_map[365] = "Earth";
	z_map[687] = "Mars";
	...
	for (ft::map<int, std::string>::iterator it = z_map.begin(); z_valuecomp(*it, *z_map.rbegin()); *it++)
		std::cout << it->first << "   " << it->second << std::endl;
